...
describe('Hasher', () => {
    ...
    it('gaci-crypto.sha256Hash should match hasher.sha256Hash', async () => {
        expect.assertions(5)
        const values: string[] = []
        for (let i = 0; i < 5; i++) {
    ...
    it('gaci-crypto.hashLeftRight should match hasher.hashLeftRight', async () => {
    ...
    it('gaci-crypto.hash3 should match hasher.hash3', async () => {
        const values: string[] = []
        for (let i = 0; i < 3; i++) {
            values.push(genRandomSalt().toString())
        }
    ...
    it('gaci-crypto.hash4 should match hasher.hash4', async () => {
        const values: string[] = []
        for (let i = 0; i < 4; i++) {
            values.push(genRandomSalt().toString())
        }
    ...
    it('gaci-crypto.hash5 should match hasher.hash5', async () => {
        const values: string[] = []
        for (let i = 0; i < 5; i++) {
            values.push(genRandomSalt().toString())
        }
